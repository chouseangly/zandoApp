        ...
        <>
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-3">
                    <button className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-50 text-sm font-medium">
                        <Upload size={16} /> Export
                    </button>
                    <button className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-50 text-sm font-medium">
                        <Download size={16} /> Import
                    </button>
                    <button className="bg-black text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-800 text-sm font-medium">
                ...
                <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
                     <div className="relative w-full max-w-sm">
                       ...
                       <input
                           ...
                           className="w-full bg-gray-50 py-2 pl-10 pr-4 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                       />
                    </div>
                    <div className="flex items-center gap-4">
                        ...
                        />
                        <button className="text-gray-600 font-medium text-sm">More Filters</button>
                    </div>
                </div>
                    ...
                    <table className="min-w-full">
                        <thead>
                            <tr className="border-b-2 border-gray-100">
                                ...
                                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Shop Rate</th>
                        ...
                        <tbody>
                            {filteredData.map((transaction) => (
                                <tr key={transaction.id} className="border-b border-gray-100 hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-2 text-sm text-gray-600">F-{transaction.id}</td>
                                    <td className="px-4 py-2"><StatusBadge status={transaction.status} /></td>
                                    <td className="px-4 py-2 text-sm text-gray-600">${parseFloat(transaction.totalAmount).toFixed(2)}</td>
                                    <td className="px-4 py-2 text-sm text-gray-600">${transaction.amountDue}</td>
                                    <td className="px-4 py-2"><ProgressBar percentage={transaction.shopRate} /></td>
                                    <td className="px-4 py-2 text-sm text-gray-600">{format(new Date(transaction.orderDate), 'dd MMM yyyy')}</td>
                                    <td className="px-4 py-2">
                                        <button onClick={() => handleViewProfile(transaction)} className="text-gray-400 hover:text-gray-700">
